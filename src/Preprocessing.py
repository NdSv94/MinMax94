...
def set_multilevel(df):
    ...
    for column in df.columns:
        ...
        if column in ('station_id', 'date_time'):
            others.add(column)
        else:
...
class Preprocessor():
    ...
    def AddSolarAngles(patterns_interpolated, solar_data_path="/home/ndsviriden/mm94/data_csv/station_def.csv"):
        ...
        def solar_to_pattern(df):
            ...
            df_solar = pd.merge(df, station_def, 'inner', on=[('station_id',)])
        ...
        patterns_with_angles = [solar_to_pattern(pattern) for pattern in patterns_interpolated if (not pattern.empty)]
